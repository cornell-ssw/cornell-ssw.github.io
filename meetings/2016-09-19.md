...
First, follow the directions for getting `ssh` setup in the
[Convenient Login to Remote Hosts](#convenient-login-to-remote-hosts) section.

Afterward, there is a presentation by Alex Townsend on everyday `git` concepts
and usage.

The end of the meeting will consist of users already familiar with clusters and
`ssh` / `git` getting setup for their usage of _Totient_.  Users who want to
get more hands-on practice with `git` can refer to the
[Configuration is Key](#configuration-is-key) section at the end.
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
      <h4 class="panel-title">
        <a data-toggle="collapse" href="#sshcommand"><tt>ssh</tt> - Secure SHell</a>
      </h4>
    </div>
    <div id="sshcommand" class="panel-collapse collapse">
      <div class="panel-body">
        <p><tt>ssh [opts] &lt;username@remote.host&gt;</tt></p>
        <ul>
          <li><tt>username</tt> is the username on the <i>remote</i> host.</li>
          <li><tt>remote.host</tt> is the url of the server you want to log into.</li>
          <ul>
            <li>IP Address: <tt>128.253.51.103</tt></li>
            <li>Symbolic name: <tt>en-cs-totient-01.coecis.cornell.edu</tt></li>
          </ul>
          <li>Use <tt>-l</tt> to specify username (no need for <tt>@</tt> anymore).</li>
          <li><tt>-p &lt;port&gt;</tt>: connect to a specific port (may be necessary depending on the server).</li>
          <li>Can forward graphical <i>programs</i> (NOT the entire screen):</li>
          <ul>
            <li>Enable <tt>X11</tt> forwarding with <tt>-X</tt>.</li>
            <li>Enable "trusted" <tt>X11</tt> forwarding with <tt>-Y</tt> (actually less secure, only use if needed).</li>
          </ul>
        </ul>
      </div>
    </div>
...
*Abstract*: _Git is a version control system that is changing the way
...
Since `svn` is centralized (`svn`) vs decentralized (`git`).  In general, you
will be good to go if you use
